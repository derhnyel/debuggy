...
def main(stdscr):
    ...
    move = None
    for j in range (1000):
        mypad.addstr(mypad_pos+j,mypad_shift,str(j))
    ResultWindow.addstr(y-5,x-10,'↓↓↓')
    # for i in range(y//2-4,y//2+5):
    #     ResultWindow.addstr(i,x-10,'|')
    #     ResultWindow.addstr(i,x-8,'|')
    ResultWindow.addstr(1,x-10,'↑↑↑')    
    while True:
        key= ResultWindow.getch()
        mypad.refresh(mypad_pos, mypad_shift, 3, 6, rows-1, columns-1)
        if key ==ord('q'):
            break
        elif  key == curses.KEY_DOWN and mypad_pos!=10000:
            move='down'
            mypad_pos += 1
            mypad.refresh(mypad_pos, mypad_shift, 3, 6, y-6, x-1)
        elif key == curses.KEY_UP and mypad_pos!=0:
            move='up'
            mypad_pos -= 1
            mypad.refresh(mypad_pos, mypad_shift, 3, 6, y-6, x-1)
