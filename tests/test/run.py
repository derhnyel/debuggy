...
def _get_caller_script():
        ...
        script = linecache.getlines(_get_caller_stack().filename)
        #frame_stack = inspect.stack()
        #print(frame_stack)
        #script.pop(3)
...
def _main():
    ...
    os.system('start cmd /K %s %s\main.py %s %s %s'%(__python_path__,__module_path__,process_id,os.path.join(_caller_path,_get_caller_stack().filename.replace('.py','.err')),os.path.join(_caller_path,_get_caller_stack().filename)))
